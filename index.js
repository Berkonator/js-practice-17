...
function replaceText(word, replacement, text) {
  const newText = text.replace(word, replacement);
  return newText;
...
function countEmails(str) {
  const regex = new RegExp(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g);
...
function replaceWordOccurrences(str, word, newWord) {
  const regex = new RegExp (str.replaceAll(word, newWord),'g');
  return regex
...
function checkRegexMethods(regex) {
  return [ 'dotAll', 'multiline', 'sticky' ]
